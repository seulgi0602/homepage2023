	...
	<%-- c tag 설명 --%>
		...
		<c:when test="${step eq '2' }">
			<li>
				...
				<%--<c:import url="/temp/selectList.do" chaarEncoding="utf-8/> --%>
			</li>
		</c:when>
		...
		<c:when test="${step eq '5' }">
			<li>
				<h3>c:choose,c:when,c:otherwise Tag :조건문중에 하나. JAVA에서 사용하는 IF,else if,else와 비슷함</h3>
				<h4>기본문법:<br/>
				    &#60;c:choose&#62;<br/>
		...
		<c:when test="${step eq '10' }">
			<li>
				<h3>fn:indexOf():주어진 문자열이 특정 문자열로 시작는지 확인하는데 사용</h3>
		...
		<c:when test="${step eq '11' }">
			<li>
				...
				<c:if test="${fn:startsWith(String,'Welcom') }">
		...
		<c:when test="${step eq '19' }">		
			<li>
				...
				<p>String-2 Legnth is :${fn:length(str2)}</p>
